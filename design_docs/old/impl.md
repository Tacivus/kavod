...
Every failure arm first calls `cancel_startup(services)`, which atomically commits `Unpublished -> StartupCancelled` and wakes every previously successful worker blocked in `wait_until_published`. Only then does it invoke reverse `rollback_one` calls, allowing each earlier Port to join an already-released worker and satisfy quiescence. The failed Port cannot depend on that later cancellation: its `start(Err)` contract requires it to clean up all activity it started before returning the error.

`start_one`, `cancel_startup`, and `rollback_one` are generic library helpers that own session-state checks, error normalization, cancellation publication, and quiescence contracts. The topology code owns only concrete forward calls, boolean progress, and concrete reverse calls. It uses no trait object, erased pointer, stored heterogeneous borrow, or unsafe code. After failure, generic cleanup closes every endpoint that was never moved into a successful worker.
...
pub struct SimContext<'a, C: PortContract> {
    now: LogicalTime,
    cursor: &'a mut Option<LogicalTime>,
    failure: &'a mut Option<ReportedSimContextFailure>,
...
Runtime `set_next` and `clear_next` immediately submit the first detected failure through `reporter`; successful Fatal-inbox insertion establishes its global report order at detection time. They then store `ReportedSimContextFailure` locally so later Context calls do not duplicate it and the generic callback wrapper knows that callback output must not be staged. Startup contexts have no run Fatal authority and instead latch the first failure into the pre-run startup result.
...
When a callback both returns an implementation error and detects a Context failure, the Context/Core failure was already reported during callback execution; the implementation error is reported after return and is retained as a secondary report when capacity permits. Startup uses the same deterministic precedence in its bounded startup error.
...
The topology-specific match contains no error policy. Each arm calls one generic typed wrapper with these conceptual signatures:

```rust
fn __call_sim_start<C, P>(
    port: &mut P,
    endpoint: &mut BindingSlotQueues<C>,
    slot: SlotOrdinal,
...
`__call_sim_command` validates endpoint identity, checks and increments the same-time callback count before dequeue, pops exactly one typed envelope, constructs `SimContext`, invokes `on_command` with the inner Contract Command, and reports any returned `RuntimeError` after any Context report. It returns `Completed` only if neither failed.

`__call_sim_step` validates endpoint identity and the selected cleared cursor, checks and increments the callback count before invocation, constructs `SimContext`, and invokes `step`. A Context or returned Runtime failure prevents callback output staging and returns `FatalReported`. `None` returns `NoEvent`; `Some(Event)` is staged through that endpoint and its staging result is normalized.

`__call_sim_start` validates the supplied Slot against endpoint-owned identity and checks its local startup Context failure before success. `__call_sim_stop` constructs the restricted stop context, invokes the callback, normalizes its StopError, and validates complete Command disposition even when the callback returned an error.
