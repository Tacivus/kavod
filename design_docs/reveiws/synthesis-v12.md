...
### Theme 1: Environment contract — shutdown, latch, and quiescence semantics

§5's preamble claims "This section is the complete contract." The clusters below are the places where that claim outruns the text: orderings and terms a bespoke implementor needs that no row fixes.
...
### Theme 2: Enforcement-tier and verification completeness

§0: "Every ID outside the Obligations table is **enforced**: violation is unrepresentable, panics an always-on assertion, or is pinned by a named test suite." §12: "This table is the complete trusted boundary — an obligation absent from it is enforced, not assumed." The clusters below are the rows for which no tier can be located. One shape, many instances: the fix is a per-row tier accounting (assign suites, add `TRUST-*` rows, or scope §0's quantifier), and most clusters below close under it.
...
### Theme 3: The Run — grammar scope and edge taxonomy

The graph itself held everywhere (see Held under fire). What survives here is the seam between the binding tables and the fused realization, and overclaims in the grammar's own scoping sentences.
...
### Theme 4: Live/Sim Port-facing API gaps
...
### Theme 5: Panics and the test profile
...
### Theme 6: Vocabulary and self-conformance
