...
1. **API blocks** — item names, type shapes, trait bounds, and variant sets are exact,
   ...
   required; further derives are free, and receiver style is free except where a block
   shows a consuming receiver, which binds. A block a section marks provisional binds
   its semantics only, until the Wiring section closes.
...
or a certificate can carry a rule, it must. Always-on constant-time assertions cover where
types run out. Tests cover the rest.
...
| A4 | First failure wins | The first Error or fatal Core condition the run observes is the Fatal cause. Once a first Error or fatal Core condition exists — or, on a run that ends without one, once the latch has closed — everything after is best-effort cleanup whose Errors are discarded. |
...
| A9 | Determinism | The Core introduces no choice of its own: every run output is a function of the build, the Application, its initial State, the configuration, and the trace. |
...
that panic on violation (A8). Their reach follows the enforcement order: unrepresentable,
then asserted, then tested.
...
| `ENV-LATCH` | The latch holds at most the first published Error. States: empty → pending on the first publication; pending → reported when `take_error` returns it or an operation returns it as its `Err`; empty, pending, or reported → closed when shutdown's close runs — a pending Error at the close leaves through the report, the run's final latch observation. Every publication after the first, and every publication after the close, is discarded. Publication is linearized against `next_event` and `dispatch` commitment, `take_error`'s snapshot, and the close: a publication linearized before an operation's own commitment is taken, marked reported, and returned as that operation's `Err`; one linearized after it leaves the operation's result standing and stays pending. An operation that fails before its commitment is not an observation point: it returns its own Error and a concurrent publication stays pending. A `next_event` call waiting for input returns once the latch is pending. |
...
| `JRN-ENCODE` | Encoding completes in the reusable bounded buffer before any byte of that record reaches the sink. `Encode`, `NotAnObject`, and `BoundExceeded` therefore write nothing and poison nothing. The bounded buffer's zero-progress rejection is `BoundExceeded`; every other encode failure is `Encode`. |
...
| 3 | Mint the certificate at `Initial`, consuming the Journal. | — |
...
| `BetweenTurns` | The index-domain check (`RUN-INDEX`): certificate index equals `u64::MAX` → `Core(IndexExhausted)`, `next_event` uncalled. Then `next_event`; `Err` → `Environment(NextEvent)`. The candidate feeds the `EventAccepted` edge. |
| `StopPending` | `shutdown` — it consumes the Environment — then the report decides. Error `Some` → `Environment(Shutdown)` with the report's quiescence: a reported Error outranks `Incomplete` as cause. Error `None` with `Incomplete` → `Core(ShutdownIncomplete)`, finalization reusing the report. Error `None` with `Quiesced` → the `TurnCompleted(Stop)` edge. |
| `Closed` | Return `EngineExit::Stopped { state }`. |

**Edges** — each recorded edge commits its record or fails as `Journal(JournalFatal)`
carrying that record's kind; `EventAccepted` alone can also fail as
`Core(TimeRegression)`; the two recordless edges commit nothing and cannot fail. On
any failure the certificate is dropped and `RUN-FINALIZE` runs.
...
| `BetweenTurns` | `EventAccepted` | a candidate; `ENV-TIME`'s nondecrease, checked before the commit — violation is `Core(TimeRegression)`, nothing committed, and the candidate stays consumed | `TurnOpen` |
...
| `RUN-GRAMMAR` | Records are committed only through the graph's transitions, and the graph is enforced at compile time: possession of the certificate in phase S proves the Journal holds exactly the records of the certificate's path to S, and that the certificate's index and last accepted time are the run's. An out-of-order record, a record whose kind disagrees with its payload, a `TurnCompleted` outcome disagreeing with the phase's fixed answer, a caller-supplied index, a skipped checkpoint, a `CommandsDispatched` without every handoff, or a `TurnCompleted(Stop)` without a clean report is unrepresentable: each edge requirement is the phase itself, or work its own transition performs (Enforcement). The residue that stays runtime is listed there. |
...
| `RUN-FINALIZE` | Fatal finalization runs exactly once: fix the first-observed cause (A4); fix quiescence — `start` returned Ok and the Environment is unconsumed → call `shutdown`, take the report's quiescence, and discard the report's Error (A4: a cause exists); consumed, exactly when `StopPending` ran → reuse that report; `start` returned `Err` → `Quiesced` (`ENV-START`); return `EngineExit::Fatal { state, cause, quiescence }`. |
...
grammar over it is unconstructible; dropping the certificate destroys the Journal. The
`fn() -> S` marker keeps `Send`/`Sync` independent of the phase. `index()` and
`logical_time()` are the getters `Context` construction reads.

Transitions — each consumes the certificate, performs its edge's requirement itself,
commits, and returns the next phase only on success. A requirement is never a loose
value a caller could forget, reuse, or forge: it is either the phase itself or work
the transition does.

| Certificate | Transition | Does | Record | Returns |
|---|---|---|---|---|
| `Initial` | `run_started(start_time)` | — | `RunStarted` | `TurnOpen` at index 0 |
| `TurnOpen` | `no_commands()` | asserts the batch empty; infallible, no commit | — | `EffectsComplete` |
| `TurnOpen` | `dispatch_batch(env, &[C])` | asserts nonempty; commits `CommandsPrepared`; runs the whole handoff loop in order; a dispatch `Err` carries `{ position, error }` | `CommandsPrepared`, then `CommandsDispatched` only after the last handoff | `EffectsComplete`, realizing the graph's `Prepared` state internally |
| `EffectsComplete` | `checkpoint(env, answer)` | the `take_error` snapshot; `Some` consumes the certificate into the `Environment(Checkpoint)` path | — | `Checkpointed<answer>` — the answer fixed in the phase |
...
| `StopPending` | `close(env)` | `shutdown`; a report of `Some` or `Incomplete` consumes the certificate into its Fatal path | `TurnCompleted(Stop)` | `Closed` |
| `BetweenTurns` | `accept_event(time, &event)` | derives the next index; checks `ENV-TIME`'s nondecrease before committing — violation is `TimeRegression` | `EventAccepted` | `TurnOpen` |

`dispatch_batch` is one transition over one slice — with separate prepare and dispatch
calls, two independent slices could commit a `CommandsDispatched` after a partial
handoff. The graph's `Prepared` state and both its edges bind the record sequence,
which is unchanged, as are the failure outcomes: a `CommandsPrepared` commit failure
precedes any handoff; `Err` at k keeps the prefix semantics; a `CommandsDispatched`
commit failure follows every handoff.
...
even in-module. `TurnOutcome` is chosen by the transition, never its caller; a record's
`index` is the certificate's own arithmetic — `accept_event`'s payload carries the
derived next index, which becomes the certificate's on success. Transitions take the
Environment: the Run owns interaction, and the requirement they perform is the edge's.
...
- **Three points stay runtime.** The index arithmetic behind `accept_event`, backed
  ...
  and the two arguments the Engine passes from the turn it just ran: the answer to
  `checkpoint` and the batch slice to `dispatch_batch`, one call site each.
  Everything else in `RUN-GRAMMAR`'s list is unrepresentable.
- **Residual always-on asserts:** `dispatch_batch` rejects an empty slice; the
  recordless batch edge asserts the batch it bypasses is empty — a nonempty batch
...
observation; localizing the cause is Port evidence.
...
| `LIVE-SELECT` | `next_event` waits, without busy-spinning, until the latch is pending or one Event is available; the choice between them follows `ENV-LATCH`'s linearization. The stamp is taken before the dequeue, and the dequeue is the consumption commitment (`ENV-ERRORS`): nothing fallible follows it. |
...
| `LIVE-SHUTDOWN` | `shutdown` realizes `ENV-SHUTDOWN`: in one linearized instant it publishes the signal, ends `Running`, closes the fan-in, and closes the latch — taking a pending Error into the report (`ENV-LATCH`) — then wakes every Kavod-owned blocking point. It waits at most the shutdown deadline — this Environment's configured bound on shutdown waiting — for completion publications, joining publishers (prompt by construction: publication follows the Port's last work, destructors included), detaching stragglers at the deadline, and discarding their Errors (A4). The report carries `Quiesced` exactly when every supervised thread was joined. |
...
the latch lock, one linearized instant — then a timed wait against the deadline from
the monotonic clock for completion publications, joining publishers in Slot order.
...
the latch lock — waking the select.
...
trusted obligation (`TRUST-LIFECYCLE`) rather than a Kavod guarantee.
...
| `SIM-LIFECYCLE` | Any `Err` a SimPort method returns ends that Port's lifecycle: no further method, `stop` included, reaches it. |
| `SIM-START` | `start` fixes the start time from the configured origin and sets `now` to it, then calls every Port's `start` in frozen Slot order; successful return is the startup commitment (`ENV-ERRORS`). On the first `Err` it calls `stop` on every Port whose lifecycle is open (`SIM-LIFECYCLE`), in frozen Slot order, discarding their Errors, then fails startup — effects already made stay real (A4's cleanup rule), and the return satisfies `ENV-START`. |
...
| `SIM-DISPATCH` | `dispatch` synchronously routes to exactly one Port's `on_command`; the invocation is where `dispatch`'s handoff commits (commitment table), and `now` does not advance. An `Err` from `on_command` is published (`ENV-ERRORS`) and the `dispatch` returns `Ok` — the invocation already committed. |
...
| `SIM-SHUTDOWN` | `shutdown` realizes `ENV-SHUTDOWN`, and the `stop` call is the sim shutdown signal: it closes the latch into the report (`ENV-LATCH`), calls `stop` on every Port whose lifecycle is open (`SIM-LIFECYCLE`), in frozen Slot order, discarding their Errors (A4), and reports `Quiesced`. |
...
*Derive:* processing is synchronous, so quiescence is structural: the report always
carries `Quiesced` — and on the Stop path its Error is structurally `None`, because
nothing runs between the checkpoint and the close.
...
sections; the commitment table; the `ShutdownReport`; the Certificate transition set
(Enforcement); `Send + 'static` boundaries; frozen Slot order as the only ordering
authority; a nonempty Port set (`BOUND-STATIC`); nonzero configured bounds (A6,
`BOUND-NONZERO`); everything frozen before `Engine::run`.
...
| `TRUST-SPAWN` | A Port ends every activity it started — threads, callbacks, timers — before `run` returns; run-scoped activity is otherwise unwitnessable | Port author | Review |
...
- One conformance trace suite runs against both Environments and compares every
  Core-owned discriminant and payload in `DET-ENV`'s list; run against a bespoke
  Environment, the same suite is its certification (`TRUST-ENV`). It compares the
  expressible overlap: a failure shape only one Environment type can produce has no
  cross-type case.
...
- Fault injection exercises every edge: scripted sinks for Journal failures, scripted
  Environments for each operation's `Err` and for a `Some` shutdown report, checking
  the resulting `FatalCause`.
- A compile-fail suite proves illegal transition sequences, a skipped checkpoint, a
  premature `TurnCompleted(Stop)`, a partial-dispatch `CommandsDispatched`, and an
  outcome disagreeing with the fixed answer do not compile (`RUN-GRAMMAR`); it lives
  where the module-private grammar types are visible.
- Live lifecycle tests prove: no `LivePort::run` begins before gate activation; failed
  ...
  read as "joined", never "succeeded", under the unwinding test profile; deadline
  expiry returns `Incomplete` while detaching only unfinished threads.
- Environment conformance tests prove both sides of `ENV-LATCH`'s linearization,
  permanent first-Error reporting, final-Command simulated Error observation, the
  close's report of a pending Error, and that `Stopped` follows only a clean report.
...
| `BOUND-LOOPS`, `NO-UNSAFE`, `NO-UNWIND`, `BOUND-STATIC`, `BOUND-NONZERO` | Laws |
