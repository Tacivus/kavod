mod grammar_compile_fail {
    ...
    /// Invariant: a certificate in any phase cannot be forged from the Engine's
    /// position, neither by advancing a phase directly nor by building the struct
    /// from its parts.
    /// Design Doc: RUN-GRAMMAR
    #[test]
    fn certificate_forgery_does_not_compile() {
        let cases = trybuild::TestCases::new();
        cases.compile_fail("tests/grammar_fixture/cases/certificate_forgery_by_advance.rs");
        cases.compile_fail("tests/grammar_fixture/cases/certificate_forgery_by_literal.rs");
    }

    /// Invariant: once a turn's answer is fixed in its phase, no transition
    /// accepts another answer.
    /// Design Doc: RUN-ENFORCEMENT
    #[test]
    fn a_classified_turn_cannot_be_reclassified() {
        let cases = trybuild::TestCases::new();
        cases.compile_fail("tests/grammar_fixture/cases/reclassified_turn.rs");
    }
}
