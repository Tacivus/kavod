...
mod receive_only_fixture {
    ...
    #[expect(
        ...
        reason = "the fixture's Event type is private to this file, so `pub` would not compile"
    )]
...
mod ports_macro_downstream {
    ...
    const fn event_destination(event: &ConsumerEvent) -> (&'static str, u64) {
        match event {
    ...
    const fn command_destination(command: &ConsumerCommand) -> (&'static str, u64) {
        match command {
